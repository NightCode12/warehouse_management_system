...
import { Plus, Trash2, Send, Package, CalendarDays, User, AlertTriangle } from 'lucide-react';
import { supabase } from '@/lib/supabase/client';
import { getStores, getInventoryProducts } from '@/lib/supabase/queries';
...
export default function ManualEntryForm({ onSubmitted }: ManualEntryFormProps) {
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>([
    { id: lineCounter++, sku: '', productName: '', variant: '', quantity: 1 },
  ]);
  ...
  // Derive unique product names for dropdown
  const uniqueProductNames = [...new Set(inventoryProducts.map(p => p.product_name))];

  // Get variants for a given product name
  const getVariantsForProduct = (productName: string) => {
    const variants = inventoryProducts
      .filter(p => p.product_name === productName && p.variant)
      .map(p => p.variant as string);
    return [...new Set(variants)];
  };

  // Find SKU by product name + variant
  const findSku = (productName: string, variant: string) => {
    const match = inventoryProducts.find(
      p => p.product_name === productName && (p.variant || '') === variant
    );
    return match?.sku || '';
  };
            ...
            <div>
              ...
              <select
                ...
                onChange={e => {
                  ...
                  // Reset line items when store changes so stale selections don't persist
                  setLineItems([{ id: lineCounter++, sku: '', productName: '', variant: '', quantity: 1 }]);
                }}
                className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            ...
            <div className="space-y-3">
              {lineItems.map((item, idx) => {
                ...
                const variants = hasStore ? getVariantsForProduct(item.productName) : [];
                ...
                return (
                  <div key={item.id} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                    ...
                    <div className="grid grid-cols-12 gap-2 flex-1">
                      {/* SKU — auto-filled when store mode, manual otherwise */}
                      <input
                        type="text"
                        ...
                        readOnly={hasStore}
                        className={`col-span-3 px-3 py-2 border border-slate-300 rounded-lg text-sm font-mono text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 ${hasStore ? 'bg-slate-100 cursor-not-allowed' : ''}`}
                      />

                      {/* Product Name — dropdown when store selected */}
                      {hasStore ? (
                        <select
                          ...
                          onChange={e => {
                            ...
                            // Reset variant and SKU when product changes
                            const prodVariants = getVariantsForProduct(name);
                            if (prodVariants.length > 0) {
                              updateLineItem(item.id, 'variant', prodVariants[0]);
                              updateLineItem(item.id, 'sku', findSku(name, prodVariants[0]));
                            } else {
                              updateLineItem(item.id, 'variant', '');
                              updateLineItem(item.id, 'sku', findSku(name, ''));
                            }
                          }}
                          className="col-span-4 px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                        >
                        ...
                        <input
                          ...
                          required
                          className="col-span-4 px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                      )}

                      {/* Variant — dropdown when store selected and product has variants */}
                      {hasStore ? (
                        <select
                          value={item.variant}
                          onChange={e => {
                            ...
                            updateLineItem(item.id, 'sku', findSku(item.productName, v));
                          }}
                          disabled={!item.productName || variants.length === 0}
                          className={`col-span-3 px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 ${!item.productName || variants.length === 0 ? 'bg-slate-100 cursor-not-allowed' : ''}`}
                        >
                          {variants.length === 0 ? (
                        ...
                        <input
                          ...
                          className="col-span-3 px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                      )}
                      ...
                      <input
                        ...
                        className="col-span-2 px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-800 text-center focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <button
                      type="button"
